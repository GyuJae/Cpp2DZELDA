...
void Player::Update()
{
	...
	if (GET_SINGLE(InputManager)->IsButton(KeyType::SpaceBar))
	{
	...
	if (GET_SINGLE(InputManager)->IsButtonUp(KeyType::SpaceBar))
	{
		_playerTurn = false;
	...
	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::KEY_1))
	{
		GET_SINGLE(UIManager)->SetSpecialWeapon(false);
	}

	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::KEY_2))
	{
