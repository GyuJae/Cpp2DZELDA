...
{
	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::LeftMouse))
	{
		...
		if (this->_setOrigin)
		{
			...
			this->_setOrigin = false;
		}
		else
		{
			this->_lines.push_back(make_pair(this->_lastPos, mousePos));
			this->_lastPos = mousePos;
		}
	}

	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::RightMouse))
	{
		this->_setOrigin = true;
	}

	// 저장
	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::S))
	{
		wofstream file;
	...
	// Load
	if (GET_SINGLE(InputManager)->IsButtonDown(KeyType::D))
	{
		wifstream file;
		file.open(L"Unit.txt");
...
void EditScene::Render(HDC hdc)
{
	for (auto& line : _lines)
	{
		...
		Pos pos1;
		pos1.SetX((float)pt1.x);
		pos1.SetY((float)pt1.y);

		Pos pos2;
		pos2.SetX((float)pt2.x);
		pos2.SetY((float)pt2.y);
